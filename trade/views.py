...
class TradeTradingReport(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if monthly_trade.exists():
            ...
            qs = qs.annotate(percent_margin=ExpressionWrapper(100*self.df*F('gross_profit') / F('sales'), 
                                                        output_field=DecimalField(decimal_places=2)
                                                        ))
            qs = qs.annotate(percent_sales=ExpressionWrapper(100*self.df*(F('indirect_expenses') + F('direct_expenses')) / F('sales'), 
                                                        output_field=DecimalField(decimal_places=2)
                                                        ))
            ...
            current_year_total = {
                ...
                'percent_margin': qs.aggregate(total=Avg('percent_margin'))['total']/self.df,
                'percent_sales': qs.aggregate(total=Avg('percent_sales'))['total']/self.df,
                'percent_gross': qs.aggregate(total=Avg('percent_gross'))['total']/self.df,
                'percent_purchase': qs.aggregate(total=Avg('percent_purchase'))['total']/self.df,
